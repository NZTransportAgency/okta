...
class OktaRequestFilter implements RequestFilter
{
    ...
    public function preRequest(SS_HTTPRequest $request, Session $session, DataModel $model)
    {
        ...
        // If we've reached here, then the user needs to log in so we send them off to Okta
        // I couldn't find a nicer way to do this in SilverStripe
        // First, store the current URL so we can redirect back when they've logged in
        $session->set('OktaBackURL', $request->getURL(true));
        $session->save();
        header("Location: " . $okta->getLoginUrl());
